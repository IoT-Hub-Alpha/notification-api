...
from .services.helper import construct_paginated_response
from .models import NotificationTemplate, NotificationDelivery
from iot_auth.django import CheckPermissionsMixin, check_permissions
...
def handle_api_errors(view_func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(
                "ApiValidationError",
                extra={"errors": e.errors, "status": e.status_code},
            )
            return JsonResponse({"errors": e.errors}, status=e.status_code)
        except BadRequestError as e:
            logger.error(
                "BadRequestError", extra={"errors": e.message, "status": e.status_code}
            )
            return JsonResponse({"error": e.message}, status=e.status_code)
        except NotFoundError as e:
            logger.error(
                "NotFoundError", extra={"errors": e.message, "status": e.status_code}
            )
            return JsonResponse({"error": e.message}, status=e.status_code)
...
class NotificationTemplateListView(CheckPermissionsMixin, View):
    permission_map = {
    ...
    def get(self, request: AuthenticatedHttpRequest) -> JsonResponse:
        ...
        try:
            ...
            return JsonResponse(
                {"error": "page_size must be a valid number"}, status=400
            )
        ...
        data = [
            NotifTemplateSerializer(instance=obj).to_dict()
            for obj in page_obj.object_list
        ]
        response = construct_paginated_response(data, page_obj, page_size, paginator)
        return JsonResponse(
            response,
            status=200,
        )
    ...
    def patch(self, request: HttpRequest) -> JsonResponse:
        ...
        return JsonResponse(
            NotifTemplateSerializer(instance=updated_instance).to_dict()
        )


@method_decorator(csrf_exempt, name="dispatch")
class NotificationDeliveryListView(CheckPermissionsMixin, View):
    permission_map = {
        "get": ["notifications.view"],
        "post": ["notifications.add"],
        "patch": ["notifications.patch"],
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        data = [
            NotifDeliverySerializer(instance=obj).to_dict()
            for obj in page_obj.object_list
        ]
        response = construct_paginated_response(data, page_obj, page_size, paginator)
        return JsonResponse(
